...
export class PassportConfig {
  static setupPassport(passport: PassportStatic, model: Model<IUserModel>): void {
    ...
    const checkSignIn: localStrategy.VerifyFunction = function(email: string, password: string, done) {
      process.nextTick(() => {
    ...
    const checkSignUp: localStrategy.VerifyFunction = (email: string, password: string, done) => {
      process.nextTick(() => {
